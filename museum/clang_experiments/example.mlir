func.func @loop_add(%init : index, %lb : index, %ub : index, %step : index) -> index {
...
func.func @main() -> i32 {
    ...
    %out = call @loop_add(%init, %lb, %ub, %step): (index, index, index, index) -> index
    %out_i32 = arith.index_cast %out : index to i32
    func.return %out_i32 : i32
}

func.func @main2(%init : index, %lb : index, %ub : index, %step : index) -> i32 {
    %out = call @loop_add(%init, %lb, %ub, %step): (index, index, index, index) -> index
    %out_i32 = arith.index_cast %out : index to i32
    func.return %out_i32 : i32
}
